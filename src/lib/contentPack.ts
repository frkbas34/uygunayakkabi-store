    ...
    if (hasUsableContent && result.commercePack) {
      ...
      try {
        ...
        if (updatedProduct && shouldAutoTriggerAudit(updatedProduct)) {
          // D-181: If content was regenerated by mentix auto-fix, mark re-audit as 'auto_retry'
          // to prevent infinite audit→fix→audit loops
          const auditSource = triggerSource === 'mentix_auto_fix' ? 'auto_retry' as const : 'auto_content_ready' as const
          triggerAudit(payload, updatedProduct, auditSource, req).catch((auditErr: unknown) => {
            console.error(
