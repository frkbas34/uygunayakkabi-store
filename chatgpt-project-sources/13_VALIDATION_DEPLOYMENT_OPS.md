...
Current as of 2026-07-27. Latest deployed feature boundary: D-501. Production serves READY Vercel deployment `dpl_7Qo8AUvrTcs4RbThdyaG6TGzEiCf`; READY deployment `dpl_8LtCEGe3ssrwGcf47grCwz3WQWZR` is the rollback candidate. Both are Vercel Redeploy results from unchanged old commit `8adfd1b955baf534da2b20595e6cdd2a407438fe`. Production, Preview, and Development each have exact `PAYLOAD_DB_PUSH=false` and the restricted replacement pooled database credential. The lineage runtime remains unpushed and undeployed. The reviewed production expansion independently verifies as `ALL_SEVEN_COLUMNS_PRESENT_COMPATIBLE`.
...
The local production build compiles with the additive fields. The production database contains the exact nullable lineage columns, so the schema prerequisite is complete. The old runtime remains active and can omit every new column. `project-control/NEON_CREDENTIAL_REPLACEMENT_V1.md` closes the credential-handling exception with replacement direct/pooled proof, two healthy unchanged-runtime redeployments, and old-credential rejection evidence. Runtime push/deployment still requires separate authorization.
...
The target tables remained small by catalog estimate (119 image jobs / 458,752 bytes and 641 Media records / 1,417,216 bytes). The authorized `ACCESS EXCLUSIVE` interval completed without contention under the migration's 5-second lock timeout and 30-second statement timeout; no retry occurred. Exact next task: `PUSH AND DEPLOY DURABLE IMAGE SLOT IDENTITY RUNTIME V1`. The credential and unchanged-runtime health gates are renewed, but runtime push/deployment still requires its own authorization.

Main pushes create Vercel production-target deployments directly. The `d832302` governance deployment was automatically created and then canceled by the ignored-build step, so GitHub's successful `Canceled by Ignored Build Step` status is not deployment of that commit. The runtime-bearing local chain changes `src/**`/`package.json` and would proceed to a production build if pushed. The two credential-remediation redeployments rebuilt only the unchanged old deployment source; they did not upload local code. Durable-runtime push, deployment, promotion, and live smoke remain separately unauthorized.
