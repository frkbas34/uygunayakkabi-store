...
**Phase 20A Instagram/Facebook Dispatch — RESOLVED + VERIFIED** (2026-04-05) — Root causes identified and fixed for Instagram/Facebook dispatch failures on product #180. (1) ROOT CAUSE: Facebook Page "UygunAyakkabı" (ID: 1040379692491003) was DEACTIVATED in Meta Business Suite — caused all Graph API calls to fail with error 100/33. FIX: Page re-activated via Meta Business Suite. Instagram Business Account ID `17841443128892405` confirmed valid (username: uygunayakkabi_34). (2) ROOT CAUSE: afterChange hook passed `doc` at depth=0 to dispatchProductToChannels — images[].image contained bare IDs (e.g., 686) instead of populated objects with url/filename. extractMediaUrls() returned empty array, causing both IG and FB direct API paths to fail the `mediaUrls.length > 0` check and fall through to webhook path. FIX: Added `req.payload.findByID({ depth: 1 })` before dispatch (commit ca4ccad). (3) VERIFIED: Env vars INSTAGRAM_PAGE_ID and BLOB_READ_WRITE_TOKEN were present all along (set Mar 10/Mar 22). Vercel Blob storage working — files exist and are publicly accessible. Payload static handler `/api/media/file/` serves images correctly (HTTP 200, proxies from Blob). (4) MANUAL API VERIFICATION: Instagram container creation + publish succeeded (postId=18016666670834577). Facebook page photo post succeeded (postId=1040379692491003_122103937328884171). Both using product #180's image. D-118.
