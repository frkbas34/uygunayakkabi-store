...
check('urgency snapshot renders stale lead, stale order, soldout, and low-stock signals', () => {
  ...
  assert.ok(rendered.includes('/leadplan'))
  assert.ok(rendered.includes('/orderreminders'))
  assert.ok(rendered.includes('/inbox stock'))
  assert.ok(!rendered.includes('/shopier publish-ready confirm'))
  assert.ok(!rendered.includes('/adlaunch'))
})
