...
| 9. Deployment and ops | Runbooks, release candidate, PR package, safe smoke governance, and a secret-safe provider/webhook Operator Evidence Record are tested. The D-380-D-500 stack is committed on `codex/master-build-plan-d500` and rebased on `origin/main`. | Operator approval to push and open a draft PR, then deployment, rollback proof, and operator-present live smoke. |
...
2. Review the prepared local branch, then separately approve its push and draft PR.
