...
The production-expansion pre-flight remains `PRODUCTION_PREFLIGHT_FAIL_SCHEMA_PUSH_UNSAFE`. Authenticated Vercel metadata proves `PAYLOAD_DB_PUSH` is absent from preview and production, and the serving old runtime still interprets absence as enabled. The local fix does not alter that deployed truth. The single encrypted `DATABASE_URI` record spans development, preview, and production; Neon project/branch, PostgreSQL major version, backup/PITR, a direct migration endpoint, the dedicated read-only metadata gate, and the seven-column production fingerprint remain unproven. `project-control/IMAGE_SLOT_LINEAGE_PRODUCTION_PREFLIGHT_V1.md` is the canonical evidence package. Exact next task: `PRODUCTION PAYLOAD SCHEMA PUSH CONTROL-PLANE REMEDIATION V1`.
