...
| D-257 | Homepage/listing → PDP clickthrough polish — sticky CTA on mobile, product card "Detay" button, category chips on homepage | `src/app/(app)/UygunApp.jsx` |
| D-258 | Homepage trust / order flow clarity — 3-step inquiry flow strip, WhatsApp trust badge, "Nasıl Çalışır" scroll anchor, brand copy applied | `src/app/(app)/UygunApp.jsx` |
| D-259 | Catalog browse clarity polish — dynamic heading per filter, always-visible count, sort controls, category-scoped size filter, improved empty state | `src/app/(app)/UygunApp.jsx` |
| D-260 | Mobile catalog filter drawer — sticky compact bar + bottom sheet, active filter pills, body scroll lock, desktop layout unchanged | `src/app/(app)/UygunApp.jsx` |
