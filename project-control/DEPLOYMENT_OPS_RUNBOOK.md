...
- Before a controlled Visual Lock pilot, run `npm run smoke:visual-pilot-target:read -- --product=<id-or-sn> --confirm-read-only` with secret-safe process-provided environment values. It exhaustively reads the target's jobs, attempts, queue receipts, Media lineage, BotEvents, and StoryJobs and performs bounded HTTPS reads of ordered original Media (15-second total timeout, three validated redirects, 10,000,000-byte cap, Sharp decode). It never prints Media URLs or digests and never writes, queues, calls providers, sends Telegram, approves, attaches, publishes, dispatches, or calls Shopier. Stop unless the verdict is `TARGET_READY_FOR_PILOT_APPROVAL`; `TARGET_BLOCKED` and `TARGET_EVIDENCE_UNSUPPORTED` are nonzero stop states.
