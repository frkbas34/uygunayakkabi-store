...
export async function triggerContentGeneration(
  ...
  triggerSource: 'auto_confirmation' | 'telegram_command' | 'admin' | 'retry' | 'mentix_auto_fix',
  req?: any,
      ...
      ;(async () => {
        try {
          ...
          if (readiness.level === 'ready') {
            ...
            notifyGeoBot(
              MENTIX_GROUP_ID,
              `✅ <b>Ürün #${product.id} — ${statusLabel}</b>` + igSnippet,
          ...
          // Pipeline failed — fall back to showing the button
          console.error('[contentPack] Auto-publish pipeline failed:', pipelineErr instanceof Error ? pipelineErr.message : String(pipelineErr))
          notifyGeoBot(
            MENTIX_GROUP_ID,
            `✅ <b>Ürün #${product.id} — ${statusLabel}</b>` + igSnippet,
            [[{ text: '🚀 Yayına Al', callback_data: `geo_activate:${product.id}` }]],
          ).catch(err => console.error('[contentPack] GeoBot fallback notification failed:', err))
        }
      })()
    }
