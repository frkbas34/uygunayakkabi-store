...
- Paused live task: `CONTROLLED PRODUCTION DURABLE SLOT IDENTITY SMOKE V1`. Provider and end-to-end image workflow behavior is not yet production-smoke-proven. The clean black suede loafer fixture is the recommended candidate, but only a separately authorized operator may send the command in the private Uygunops DM.
