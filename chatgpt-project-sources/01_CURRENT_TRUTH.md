...
Verified 2026-07-28 from the checkout, fresh `origin` metadata, local validation, authenticated control-plane checks, passive production verification, the governed catalog cleanup, the controlled durable-slot production smoke, and the read-only Generated Media retention census.
...
Generated Media retention policy V1 is now defined but not wired into runtime. The production read-only census classified 527 generated Media across 127 jobs: 174 permanent business assets, 203 pending operator decisions, 136 legacy manual-review records, 9 failure-evidence records, and 5 smoke-evidence records. Complete job/attempt/slot lineage is absent on 513 historical Media. Both quarantine-proposal and physical-cleanup candidate counts are zero, and the pure classifier can never authorize deletion. See `project-control/GENERATED_MEDIA_RETENTION_AND_RECOVERABLE_CLEANUP_POLICY_V1.md` and `project-control/GENERATED_MEDIA_RETENTION_DRY_RUN_V1.md`.
...
- Rejected, regenerated, and partially approved outputs still lack persisted lifecycle, holds, quarantine, restore, cleanup-batch, and tombstone state. Policy and dry-run foundations now fail closed; no cleanup runtime is enabled.
...
- The controlled durable-slot smoke is production-proven for one exact five-slot success-and-rejection path. It does not prove broad visual quality, provider reliability, cost, every failure mode, or repeated-run behavior. Exact next task: `GENERATED MEDIA RECOVERABLE QUARANTINE FOUNDATION V1`.
