...
check('19 runtime wiring retains transient bytes through evidence and drops them before Media', () => {
  ...
  assert.match(task, /const failQualitySlot =/)
  ...
  assert.ok(task.indexOf('const failQualitySlot =') < task.indexOf('persistGeneratedSlotEnvelopes({'))
  assert.match(task, /const readRetryAuthorizationSnapshot = async/)
  assert.match(task, /areGenerationAttemptHistoriesSemanticallyEqual/)
  assert.match(task, /activeAttemptId === rootAttemptId/)
  assert.match(task, /const retrySnapshot = await readRetryAuthorizationSnapshot\(slotId\)/)
  assert.match(task, /packState: retryProcessingBlocked \? 'unrecoverable' : 'recoverable'/)
  assert.match(task, /await persistVisualLockV01AttemptRecords\(\[attemptMetadata, retryAttempt\]\)[\s\S]*const executionSnapshot = await readRetryAuthorizationSnapshot\(slotId\)[\s\S]*executeSingleVisualQualityRetryProviderV01/)
  assert.doesNotMatch(task, /persistVisualLockV01AttemptRecords\(\[retryAttempt, attemptMetadata\]\)/)
  assert.equal((task.match(/executeSingleVisualQualityRetryProviderV01\(/g) ?? []).length, 1)
  assert.match(provider, /execution\.onGenerationCallStarted\(\)[\s\S]*callGPTImageEdit/)
  assert.match(provider, /execution\.onGenerationCallStarted\(\)[\s\S]*callGeminiImageGenerate/)
  assert.equal((provider.match(/execution\.onEvaluatorCallStarted\(\)/g) ?? []).length, 2)
  assert.match(task, /\[slotDefinition\.displayOrder\]/)
  assert.match(task, /failFast: isVisualLockV01Context\(visualLockContext\)/)
  assert.match(contracts, /Retry-aware approval requires a complete validated pack-selection manifest/)
  assert.match(contracts, /cannot select a pack from non-terminal or non-passing evidence/)
  assert.match(provider, /function resolveVisualQualityRetryContextV01/)
  assert.equal((provider.match(/buildVisualQualityRetryDirectiveV01\(\{ context: qualityRetryContext/g) ?? []).length, 2)
