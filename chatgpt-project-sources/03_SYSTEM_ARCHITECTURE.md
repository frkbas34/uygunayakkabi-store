...
Next.js 16.2 canary and Payload 3.79 run one application. PostgreSQL is the database, Vercel Blob is production media storage when configured, and Payload jobs persist queued work. The pure policy resolves before the PostgreSQL adapter: missing, empty, and exact `false` disable Payload schema push; invalid values fail; and exact `true` requires an explicit second confirmation in local development and is forbidden in Vercel, CI, production, builds, tests, and read-only operations. Vercel Production, Preview, and Development each set exact `PAYLOAD_DB_PUSH=false`; production serves READY remote-Git deployment `dpl_EtChj9RhyqpAuy3M7C18BdX24Mnz` at durable-runtime commit `e0b60f6c83f6fa6d59dd6647558eca6883acb341`. READY old-runtime deployments `dpl_7Qo8AUvrTcs4RbThdyaG6TGzEiCf` and `dpl_8LtCEGe3ssrwGcf47grCwz3WQWZR` are retained for runtime-first rollback.
