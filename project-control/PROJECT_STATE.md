...
| D-269 | PDP gallery usability polish V1 — mobile touch swipe (50px delta); image fade-in animation (key+@keyframes 0.22s); thumbnail row overflowX auto (no clip); active thumbnail outer glow ring; inactive opacity 0.5→0.65 + visible border ring | `src/components/ProductImages.tsx` |
| D-270 | PDP fullscreen image inspection V1 — tap main image to open dark-backdrop lightbox; fade-in overlay; X button + backdrop tap + Escape to close; body scroll lock; swipe/arrows reused from D-269; Büyüt affordance hint; cursor zoom-in on inline image | `src/components/ProductImages.tsx` |
| D-271 | Mobile image loading performance V1 — `loading="lazy"` on catalog cards, cart thumbnails, PD thumbnails, similar-products section; `fetchPriority="high"` + `loading="eager"` on PDP hero + ProductDetail main image; hero Unsplash `fetchpriority="high"` | `src/components/ProductImages.tsx`, `src/app/(app)/UygunApp.jsx`, `src/app/(app)/products/[slug]/page.tsx` |
| D-272 | Cart/checkout expectation clarity V1 — cart drawer process note + "WHATSAPP İLE TALEBİNİZİ İLETİN" (was "SİPARİŞ VER"); ProductDetail process hint under trust badges; PDP primary CTA "SEPETE EKLE" (no-op button) replaced with `<a href="#inquiry-form">` "TALEBİNİZİ OLUŞTURUN" | `src/app/(app)/UygunApp.jsx`, `src/app/(app)/products/[slug]/page.tsx` |
| D-273 | ContactForm validation/submission confidence V1 — client-side validation (name ≥2 chars, phone regex); field-level inline errors with red borders; "(zorunlu)" on required fields; phone helper text; API 400 error body parsed for specific phone message; error box moved above submit; network vs server error distinguished; loading text improved | `src/components/ContactForm.tsx` |
| D-274 | Header/navigation/sitewide entry clarity V1 — desktop nav active-page underline indicator; mobile menu inactive items muted + active underline + › chevron; mobile menu "Yardım & İletişim" section label above WA button; WA button renamed "WhatsApp ile İletişim Kur"; footer "Sipariş" → "Yardım" | `src/app/(app)/UygunApp.jsx` |
| D-275 | Help/FAQ/Contact destination V1 — new `contact` SPA page (`HelpContactPage`) with compact 4-step process summary, 3 FAQ accordion groups (Ürün & Beden / Sipariş & Ödeme / Teslimat & Süreç), CTA block; `HelpFAQItem` accordion component; nav "YARDIM" link; mobile menu navigable "YARDIM MERKEZİ" entry; footer SSS nav link; URL `/yardim` | `src/app/(app)/UygunApp.jsx` |
| D-276 | Store credibility/about/why-us polish V1 — WHY_US_CARDS 4+5 replaced (AI system + digital presence → personal support + ordering flow); WhyUsSection subtitle de-jargoned; AboutSection AI/ops paragraphs replaced with buyer-support copy, closing "daha akıllı" → "kişisel destekle", badge updated; TrustValueSection opening "modern dijital" → "kişisel destek ve anlaşılır"; last trust bullet made specific | `src/app/(app)/UygunApp.jsx` |
| D-277 | Homepage category/intent entry polish V1 — `CategoryOverlay` moved from position 8 → position 3 (after WhyUsSection, before Popular grid); upgraded with `KATEGORİ` eyebrow + "Ne Arıyorsunuz?" heading + subtext; chip padding + icon size increased; "Tüm Ürünler →" dark fallback CTA added | `src/app/(app)/UygunApp.jsx` |
