...
| D-265 | OOS size auto-prefill / recovery UX polish V1 — new `OOSChip.tsx` client component fires `CustomEvent('oosChipClicked')` + smooth-scrolls to form; ContactForm listens via `useEffect`, auto-prefills size, shows amber contextual banner "{size} numara şu an stokta görünmüyor"; amber input border when prefill active; clears on manual edit or success reset | `src/components/OOSChip.tsx` (new), `src/components/ContactForm.tsx`, `src/app/(app)/products/[slug]/page.tsx` |
