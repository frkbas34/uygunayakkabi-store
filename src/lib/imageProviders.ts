...
function getBackgroundForColor(mainColor: string, productId?: string | number): string {
  ...
  // D-303 is the current catalog policy: all generated product cards share this background.
    ...
    for (const scene of scenes) {
      ...
      if (rawBuf) {
        ...
        if (isVisualLockV01Context(visualLock)) {
          ...
          // Retain the generated bytes only in memory so the task can always run
