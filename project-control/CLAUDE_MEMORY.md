...
- **D-325/326/327 (2026-06-18) — first ad test prep (docs only):** plan + copy/UTM pack + pre-launch runbook. Ad-safe products 359/355/354 (+353 backup), all loafers, 10 units each (353=4). Packs in `project-control/campaigns/`. UTM-only test, no pixel yet. **OPEN SHOULD-FIX:** brand-named `Louis Vuitton Loafer Bej` (358, Klasik) leaks into the "Benzer Modeller" similar-products rail on all 3 ad PDPs → operator should hide/rename 358 (+349 BOSS) before paid traffic. NEVER advertise 358/349 (trademark).
