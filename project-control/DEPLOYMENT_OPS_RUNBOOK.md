...
- Before a controlled Visual Lock pilot, run `npm run smoke:visual-pilot-target:read -- --product=<id-or-sn> --confirm-read-only` with secret-safe process-provided environment values. Numeric references bind only to Payload Product IDs; `SN...` references bind only to exact stock numbers. The verifier compares two complete Product/job/queue/product-scoped-Media/BotEvent/StoryJob/optional-authority snapshots around ordered-original inspection, and reconciles every generated Media record against jobs, attempts, semantic slots, retry lineage, approval evidence, and gallery relationships. Original HTTPS reads retain the 15-second/10,000,000-byte/40-MP per-file limits and add an 8-original, 40,000,000-byte, 160-MP, 45-second aggregate budget; Sharp runs in a worker that is terminated and awaited on timeout. It never prints Media URLs or digests and never writes, queues, calls providers, sends Telegram, approves, attaches, publishes, dispatches, or calls Shopier. Stop unless the verdict is `TARGET_READY_FOR_PILOT_APPROVAL`; `TARGET_BLOCKED` and `TARGET_EVIDENCE_UNSUPPORTED` are nonzero stop states.
