...
53. D-355 DB drift repair now has a guarded apply helper: `npm run db:imageqc:apply` previews by default, `npm run db:imageqc:apply -- --dry-run --print-sql` prints the reviewed DDL, and `npm run db:imageqc:apply -- --apply --confirm-apply-d355-image-qc-schema` is the explicit operator-run apply path. Codex/Claude should not run confirmed apply mode without operator approval. After apply, rerun `smoke:imageqc:schema` and `smoke:shopier:read`.
...
Continue D-356 Shopier/Web Publish Batch Control: first preview/apply/verify D-355 DB drift using `npm run db:imageqc:apply -- --dry-run --print-sql`, then operator-approved `npm run db:imageqc:apply -- --apply --confirm-apply-d355-image-qc-schema` if the reviewed DDL is accepted. The blocker exists because `npm run smoke:imageqc:schema -- --confirm-read-only` currently reports missing `image_quality_*` product columns plus relation `products_image_quality_defect_flags` (details in `project-control/D-355_IMAGE_QC_DB_DRIFT.md`). After apply, rerun the schema check, rerun `npm run smoke:shopier:read -- --confirm-read-only`, live operator smoke `/shopier dashboard`, `/shopier publish-ready`, `/shopier errors`, and `/shopier retry-errors`, then decide whether a broader admin batch review surface is still needed beyond the per-product Shopier gate. Ads stay paused until D-380+.
