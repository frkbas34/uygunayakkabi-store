...
Rejected, regenerated, failed-save, and unapproved slot Media lack a complete retention lifecycle. The production catalog contains 44 legacy previews older than the evidence-based review window: none has a live worker, V1 lineage, gallery attachment, or durable Telegram delivery/keyboard receipt, yet every preview still blocks current deduplication. Forty-three exact job/Media sets therefore remain pending operator decision and one invalid-reference set remains failure evidence; no cleanup is approved. Partial legacy results can also lose correct slot association because buffers and metadata are positional.
