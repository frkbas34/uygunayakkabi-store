...
export async function generateByEditing(
  ...
  try {
    const sharp = (await import('sharp')).default
    console.log(`[generateByEditing v4] input size=${referenceImage.length} mime=${referenceImageMime}`)

    // ── Step 1: ML background removal ────────────────────────────────────────
    let cutoutBuffer: Buffer | null = null

    try {
      console.log('[generateByEditing v4] starting ML background removal...')
      ...
      console.log(`[generateByEditing v4] ML removal success — cutout ${cutoutBuffer.length} bytes`)
    } catch (mlErr) {
      console.warn('[generateByEditing v4] ML removal failed:', mlErr instanceof Error ? mlErr.message : mlErr)
      console.log('[generateByEditing v4] falling back to resize+extend (no removal)')
    }

    // ── Step 2: Resize cutout (or original) to fit within 800×800 ────────────
    const sourceBuffer = cutoutBuffer ?? referenceImage
    const shoeBuffer = await sharp(sourceBuffer)
    ...
    console.log(`[generateByEditing v4] shoe scaled to ${shoeW}×${shoeH} (ML=${!!cutoutBuffer})`)

    // ── Step 3: Composite onto 5 coloured backgrounds ────────────────────────
    const composites = await Promise.all(
      COMPOSITE_BACKGROUNDS.map(({ name, r, g, b, label }) =>
        sharp({ create: { width: 1024, height: 1024, channels: 4,
          ...
          .then((canvas) =>
            sharp(canvas)
              .composite([{ input: shoeBuffer, gravity: 'centre' }])
              .flatten({ background: { r, g, b } })
              .jpeg({ quality: 92 })
              .toBuffer(),
          )
          .then((buf) => {
            console.log(`[generateByEditing v4] ✓ ${name} (${label}) — ${buf.length} bytes`)
            return buf
          })
          .catch((err) => {
            const msg = `${name}: ${err instanceof Error ? err.message : err}`
            console.error(`[generateByEditing v4] ✗ ${msg}`)
            result.errors.push(msg)
  ...
  console.log(`[generateByEditing v4] done — ${result.successCount}/${result.promptCount} images`)
