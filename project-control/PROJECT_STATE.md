...
**Phase 20 Instagram/Facebook Dispatch Validation — BLOCKED (3 BLOCKERS)** (2026-04-05) — Attempted real dispatch on product #180 (status=active, channelTargets=[website,instagram,facebook]). All three external channel dispatches failed. BLOCKER P20-1: Instagram userId `17841443128892405` is INVALID — Graph API returns error 100/33 ("Object does not exist"). Token debug confirms valid token (app: Uygunayakkabı, scopes: instagram_basic, instagram_content_publish, pages_show_list, pages_read_engagement, pages_manage_posts) but `/me/accounts` returns EMPTY — no Facebook Pages linked to the app. The Instagram Business Account cannot be resolved without a connected Page. Facebook user ID is `122097085238922240`. BLOCKER P20-2: `INSTAGRAM_PAGE_ID` env var NOT SET in Vercel production — `facebookPageId` never injected into settings snapshot, Facebook direct publish path skipped entirely. BLOCKER P20-3: ALL 459 media files stored as local relative URLs (`/api/media/file/...`), zero Vercel Blob URLs. `BLOB_READ_WRITE_TOKEN` not set — Vercel Blob storage disabled. Product #180's image URL returns HTTP 404 on production. Meta servers cannot fetch images for container creation even if userId were valid. Required operator actions: (A) Connect Facebook Page to "Uygunayakkabı" Meta app in Facebook Business Settings, re-authorize selecting the Page, get correct Instagram Business Account ID from `/me/accounts?fields=instagram_business_account`. (B) Set `INSTAGRAM_PAGE_ID`, `BLOB_READ_WRITE_TOKEN` in Vercel env vars. (C) Re-upload or re-ingest media through Blob-enabled pipeline. D-118.
